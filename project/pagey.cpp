...
PageY::PageY()
{
    setBackgroundBrush(QBrush(QPixmap("ZCY.png")));
...
void PageY::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    int x = event->pos().x();
    int y = event->pos().y();
    QPoint p(x,y);
    ...
    else if (r16.contains(p))
    {
        emit xuanGS(QString("y#6#1"));
    }
    else if (r17.contains(p))
    {
        emit xuanGS(QString("y#6#2"));
    }
    else if (r18.contains(p))
    {
        emit xuanGS(QString("y#6#3"));
    }
